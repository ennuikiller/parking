...
- (id)initWithHost:(NSString *)theHost 
		  ...
		  delegate:(id<CRVStompClientDelegate>)theDelegate {
	NSLog(@"IN CRVStompClient: initWithHost");
...
- (void)connect {
	if(anonymous) {
		NSLog(@"IN ANONYMOUS BRANCH");
		[self sendFrame:kCommandConnect];
	} else {
		NSLog(@"NOT IN ANONYMOUS BRANCH");
...
- (void)onSocket:(AsyncSocket *)sock didConnectToHost:(NSString *)host port:(UInt16)port {
	...
	NSLog(@"IN CRVStompClient onSocket:%p didConnectToHost:%@ port:%hu", sock, host, port);
}
